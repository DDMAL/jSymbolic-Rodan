...
class extract_features(RodanTask):
    ...
    input_port_types = (
...

        {
            ...
            'minimum': 0,
            'maximum': 1,
            'resource_types': ['application/txt']
        },
    )
    output_port_types = (
        {
...

        {
            ...
            'minimum': 0,
            'maximum': 1,
            'resource_types': ['application/csv']
        },
    )
    ...
    def run_my_task(self, inputs, settings, outputs):
        ...
        with self.tempdir() as temp_dir:
            ...
            src_value_path = os.path.join(temp_dir, value_file_name)
            dst_value_path = os.path.join(outputs['jSymbolic ACE XML Value Output'][0]['resource_path'], value_file_name)
            shutil.copyfile(src_value_path, dst_value_path)

            # Assign files generated by jSymbolic to definition output port
            src_definition_path = os.path.join(temp_dir, definition_file_name)
            dst_definition_path = os.path.join(outputs['jSymbolic ACE XML Definition Output'][0]['resource_path'],
                                            definition_file_name)
            shutil.copyfile(src_definition_path, dst_definition_path)

            # Try to get arff file if it exists, otherwise continue
            arff_name = "{0}.arff".format(value_file_name)
            ...
            csv_name = "{0}.csv".format(value_file_name)
